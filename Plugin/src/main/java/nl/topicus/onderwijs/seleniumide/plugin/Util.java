...
public class Util {
	public static final String HEADER_COMMENT = "// Generated by Selenium IDE to java test maven plug-in version: ";

	private static final String SEPERATOR = System
			.getProperty("file.separator");

	public static void generateJavaTests(File testcasesSourceDirectory,
			File javaTestsTargetDirectory, File javaTestTemplate,
			boolean cleanDirectories, Log log) throws MojoExecutionException {
		if (!testcasesSourceDirectory.isDirectory())
			throw new MojoExecutionException(
					"Invalid testcases source directory.");

		if (!javaTestsTargetDirectory.isDirectory())
			throw new MojoExecutionException(
					"Invalid java test target directory.");
		...
		try {
			Collection<File> testcaseFiles = FileUtils.listFiles(
					testcasesSourceDirectory, new WildcardFileFilter("*.html"),
					DirectoryFileFilter.DIRECTORY);

			log.info("Looking for Selenium IDE testcases at: "
					+ testcasesSourceDirectory);
			log.info("Generated java tests will be stored to: "
					+ javaTestsTargetDirectory);

			log.info("Generating new java test files from "
					+ testcaseFiles.size() + " testcase source files");

			int count = 0;

			for (File file : testcaseFiles) {
				List<String> lines = Files.readAllLines(javaTestTemplate
						.toPath());
				...
				String relativeSourceParentPath = asoluteSourceParentPath
						.replace(
								testcasesSourceDirectory.getPath() + SEPERATOR,
								"");
				String relativeSourceFilePath = relativeSourceParentPath
						+ SEPERATOR + file.getName();
				String asoluteTargetFilePath = javaTestsTargetDirectory
						.getPath()
						+ SEPERATOR
						+ relativeSourceParentPath
						+ SEPERATOR + targetFileName;
				String packageString = relativeSourceParentPath.replace(
						SEPERATOR, ".");

				for (int i = 0; i < lines.size(); i++) {
					lines.set(i,
							lines.get(i).replace("{package}", packageString));
	...
	public static void cleanJavaTests(File javaTestsTargetDirectory,
			boolean cleanDirectories, Log log) {
		Collection<File> javaTestFiles = FileUtils.listFiles(
				javaTestsTargetDirectory, new WildcardFileFilter("*.java"),
				DirectoryFileFilter.DIRECTORY);

		log.info("Cleaning java tests target directory that contains "
				+ javaTestFiles.size() + " files");
		...
		for (File file : javaTestFiles) {
			...
			try {
				...
				if (lines.size() == 0
						|| !lines.get(0).contains(Util.HEADER_COMMENT))
					continue;
		...
		if (cleanDirectories) {
			...
			ListIterator<File> emptyDirectoriesIterator = emptyDirectories
					.listIterator();
			...
			cleanDirectoryLog = " and " + countDeletedDirectories
					+ " empty directories";
		}

		log.info("Cleaned " + countDeletedFiles + " generated java test files"
				+ cleanDirectoryLog);
	}
}
