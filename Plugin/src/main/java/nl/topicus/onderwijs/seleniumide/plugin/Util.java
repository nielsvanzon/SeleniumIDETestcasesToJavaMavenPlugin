...
public class Util
{
	...
	public static void generateJavaTests(File testcasesSourceDirectory, File javaTestsTargetDirectory, File javaTestTemplate,
			boolean cleanDirectories, Log log) throws MojoExecutionException
	{
		...
		try
		{
			Collection<File> testcaseFiles = FileUtils.listFiles(testcasesSourceDirectory, new WildcardFileFilter("*.html"),
			...
			log.info("Generating new java test files from " + testcaseFiles.size() + " testcase source files");
			...
			for (File file : testcaseFiles)
			{
				...
				lines.add(0, Util.HEADER_COMMENT + Util.class.getPackage().getImplementationVersion());

				String className = StringUtils.capitalize(file.getName().replaceFirst("[.][^.]+$", ""));
				String targetFileName = className + ".java";
				String asoluteSourceParentPath = file.getParent();
				String relativeSourceParentPath = asoluteSourceParentPath.replace(testcasesSourceDirectory.getPath() + "\\", "");
				String relativeSourceFilePath = relativeSourceParentPath + "\\" + file.getName();
				String asoluteTargetFilePath = javaTestsTargetDirectory.getPath() + "\\" + relativeSourceParentPath + "\\" + targetFileName;
				...
				for (int i = 0; i < lines.size(); i++)
				{
					...
					lines.set(i, lines.get(i).replace("{relativeSourceFilePath}", relativeSourceFilePath.replace("\\", "/")));
				}
	...
	public static void cleanJavaTests(File javaTestsTargetDirectory, boolean cleanDirectories, Log log)
	{
		Collection<File> javaTestFiles = FileUtils.listFiles(javaTestsTargetDirectory, new WildcardFileFilter("*.java"),
				DirectoryFileFilter.DIRECTORY);

		log.info("Cleaning java tests target directory that contains " + javaTestFiles.size() + " files");
		...
		if (cleanDirectories)
		{
			...
			while (emptyDirectoriesIterator.hasNext())
			{
				...
				if (!parentDirectory.equals(javaTestsTargetDirectory) && parentDirectory.list().length == 0)
				{
